...
![blocking IO](/images/epoll/blocking-io.png)
...
![非阻塞 I/O](/images/epoll/non-blocking-io.png)
...
![I/O多路复用](/images/epoll/io-multiplexing.png)
...
![异步IO](/images/epoll/async-io.png)
...
**各个IO Model的比较如图所示：**
![IO模型对比](/images/epoll/io-model-pk.png)
...
1. 监视的描述符数量不受限制，它所支持的FD上限是最大可以打开文件的数目，这个数字一般远大于2048,举个例子,在1GB内存的机器上大约是10万左右，具体数目可以`cat /proc/sys/fs/file-max`查看,一般来说这个数目和系统内存关系很大。select的最大缺点就是进程打开的fd是有数量限制的。这对 于连接数量比较大的服务器来说根本不能满足。虽然也可以选择多进程的解决方案( Apache就是这样实现的)，不过虽然linux上面创建进程的代价比较小，但仍旧是不可忽视的，加上进程间数据同步远比不上线程间同步的高效，所以也不是一种完美的方案。
